...
int main(void)
{
	...
	AxSms::IMessagePtr oMessage = NULL, oRcvMessage = NULL;	
	...
	*/
    ...
    oMessage->BodyFormat = AXSMS_SMS_BODYFORMAT_TEXT;
    ...
    // Wait for the SMPP server for max. 5000 ms to accept the submitted SMS message. 5000 ms will usually suffice
    ...
    oRcvMessage  = oSmpp->FetchSmsUpdate();
	_tprintf( _T("FetchSmsUpdate, result: %ld (%s)\n"), oSmpp->LastError, ( LPCTSTR ) oSmpp->GetErrorDescription( oSmpp->LastError ) );
    if (oSmpp->LastError != 0 || oRcvMessage == NULL )
        goto _EndMain;
	...
	if(oRcvMessage->SmppCommandStatus!= 0)
	{
		// 1025 = nocredits            
		if( oRcvMessage->SmppCommandStatus == 1025 && _tcscmp( ( LPCTSTR ) bstrServer, _T("gateway.auronsoftware.com") ) == 0 )		
			_tprintf( _T("Message not sent. Reason: No credits left.\n"));		
		else		
			_tprintf( _T("Message not sent. Reason: unspecified error [%ld].\n"), oRcvMessage->SmppCommandStatus);		
		goto _EndMain;
	}

    // Get the message reference , that will be matched against a delivery report (still to be received) later on
    bstrReference = ( LPCTSTR ) oRcvMessage->Reference;
    _tprintf( _T("Message Reference: %s\n"), ( LPCTSTR ) bstrReference);

    _tprintf( _T("Waiting for delivery report...\n") );
    while( bWaitForDeliveryReport )
    {
        oRcvMessage = oSmpp->ReceiveMessage();
		_tprintf( _T("ReceiveMessage, result: %ld (%s)\n"), oSmpp->LastError, ( LPCTSTR ) oSmpp->GetErrorDescription( oSmpp->LastError ) );
        if( oSmpp->LastError == 0L && oRcvMessage != NULL && oRcvMessage->SmppIsDeliveryReport )
        {
			_tprintf( _T("Delivery Report found, status:%s, body:%s\n"), ( LPCTSTR ) bstrReference, ( LPCTSTR ) oRcvMessage->Body );
